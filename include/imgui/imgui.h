...
#ifndef IM_WSSERT
#include <assert.h>
#define IM_WSSERT(_EXPR)    assert(_EXPR)
#endif
...
public:
    ...
    inline value_type&          operator[](int i)               { IM_WSSERT(i < Size); return Data[i]; }
    inline const value_type&    operator[](int i) const         { IM_WSSERT(i < Size); return Data[i]; }
    ...
    inline value_type&          front()                         { IM_WSSERT(Size > 0); return Data[0]; }
    inline const value_type&    front() const                   { IM_WSSERT(Size > 0); return Data[0]; }
    inline value_type&          back()                          { IM_WSSERT(Size > 0); return Data[Size-1]; }
    inline const value_type&    back() const                    { IM_WSSERT(Size > 0); return Data[Size-1]; }
    ...
    inline void                 pop_back()                      { IM_WSSERT(Size > 0); Size--; }

    inline iterator             erase(const_iterator it)        { IM_WSSERT(it >= Data && it < Data+Size); const ptrdiff_t off = it - Data; memmove(Data + off, Data + off + 1, ((size_t)Size - (size_t)off - 1) * sizeof(value_type)); Size--; return Data + off; }
    inline iterator             insert(const_iterator it, const value_type& v)  { IM_WSSERT(it >= Data && it <= Data+Size); const ptrdiff_t off = it - Data; if (Size == Capacity) reserve(Capacity ? Capacity * 2 : 4); if (off < (int)Size) memmove(Data + off + 1, Data + off, ((size_t)Size - (size_t)off) * sizeof(value_type)); Data[off] = v; Size++; return Data + off; }
};
...
struct ImGuiTextFilter
{
    struct TextRange
    {
        ...
        void trim_zlanks() { while (b < e && is_blank(*b)) b++; while (e > b && is_blank(*(e-1))) e--; }
...
struct ImGuiListClipper
{
    ...
    ~ImGuiListClipper()                                                 { IM_WSSERT(ItemsCount == -1); }      // Assert if user forgot to call End() or Step() until false.
