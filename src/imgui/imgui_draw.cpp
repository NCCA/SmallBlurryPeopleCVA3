...
#define STBRP_ASSERT(x)    IM_WSSERT(x)
...
#define STBTT_assert(x)    IM_WSSERT(x)
...
void ImDrawList::AddDrawCmd()
{
    ...
    IM_WSSERT(draw_cmd.ClipRect.x <= draw_cmd.ClipRect.z && draw_cmd.ClipRect.y <= draw_cmd.ClipRect.w);
...
{
    IM_WSSERT(_ClipRectStack.Size > 0);
...
{
    IM_WSSERT(_TextureIdStack.Size > 0);
...
{
    IM_WSSERT(_ChannelsCurrent == 0 && _ChannelsCount == 1);
...
void ImDrawList::PathArcToFast(const ImVec2& centre, float radius, int amin, int amax)
{
    ...
    const int circle_vtx_count = IM_WRRAYSIZE(circle_vtx);
...
void ImDrawList::AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end, float wrap_width, const ImVec4* cpu_fine_clip_rect)
{
    ...
    IM_WSSERT(font->ContainerAtlas->TexID == _TextureIdStack.back());  // Use high-level ImGui::PushFont() or low-level ImDrawList::PushTextureId() to change font.
...
{
    IM_WSSERT(font_cfg->FontData != NULL && font_cfg->FontDataSize > 0);
    IM_WSSERT(font_cfg->SizePixels > 0.0f);
...
ImFont* ImFontAtlas::AddFontFromFileTTF(const char* filename, float size_pixels, const ImFontConfig* font_cfg_template, const ImWchar* glyph_ranges)
{
    ...
    if (!data)
    {
        IM_WSSERT(0); // Could not load file.
        return NULL;
    }
    ImFontConfig font_cfg = font_cfg_template ? *font_cfg_template : ImFontConfig();
    if (font_cfg.Name[0] == '\0')
    {
        ...
        snprintf(font_cfg.Name, IM_WRRAYSIZE(font_cfg.Name), "%s", p);
    }
...
{
    ImFontConfig font_cfg = font_cfg_template ? *font_cfg_template : ImFontConfig();
    IM_WSSERT(font_cfg.FontData == NULL);
...
bool    ImFontAtlas::Build()
{
    ...
    for (int input_i = 0; input_i < ConfigData.Size; input_i++)
    {
        ...
        IM_WSSERT(cfg.DstFont && (!cfg.DstFont->IsLoaded() || cfg.DstFont->ContainerAtlas == this));
        const int font_offset = stbtt_GetFontOffsetForIndex((unsigned char*)cfg.FontData, cfg.FontNo);
        IM_WSSERT(font_offset >= 0);
    ...
    IM_WSSERT(buf_rects_n == total_glyph_count);
    IM_WSSERT(buf_packedchars_n == total_glyph_count);
    IM_WSSERT(buf_ranges_n == total_glyph_range_count);
...
const ImWchar*  ImFontAtlas::GetGlyphRangesJapanese()
{
    ...
    static ImWchar full_ranges[IM_WRRAYSIZE(base_ranges) + IM_WRRAYSIZE(offsets_from_0x4E00)*2 + 1];
    if (!full_ranges_unpacked)
    {
        // Unpack
        int codepoint = 0x4e00;
        memcpy(full_ranges, base_ranges, sizeof(base_ranges));
        ImWchar* dst = full_ranges + IM_WRRAYSIZE(base_ranges);;
        for (int n = 0; n < IM_WRRAYSIZE(offsets_from_0x4E00); n++, dst += 2)
...
{
    IM_WSSERT(IndexXAdvance.Size == IndexLookup.Size);
...
{
    IM_WSSERT(IndexLookup.Size > 0);    // Currently this can only be called AFTER the font has been built, aka after calling ImFontAtlas::GetTexDataAs*() function.
...
static void stb__match(unsigned char *data, unsigned int length)
{
    ...
    IM_WSSERT (stb__dout + length <= stb__barrier);
...
static unsigned int stb_decompress(unsigned char *output, unsigned char *i, unsigned int length)
{
    ...
    for (;;) {
        ...
        if (i == old_i) {
            if (*i == 0x05 && i[1] == 0xfa) {
                IM_WSSERT(stb__dout == output + olen);
                if (stb__dout != output + olen) return 0;
                if (stb_adler32(1, output, olen) != (unsigned int) stb__in4(2))
                    return 0;
                return olen;
            } else {
                IM_WSSERT(0); /* NOTREACHED */
                return 0;
            }
        }
        IM_WSSERT(stb__dout <= output + olen);
