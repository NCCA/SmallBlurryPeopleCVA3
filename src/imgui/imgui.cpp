...
/*
 ...
 - 2014/09/24 (1.12) - moved IM_MALLOC/IM_XEALLOC/IM_FREE preprocessor defines to IO.MemAllocFn/IO.MemReallocFn/IO.MemFreeFn
...
ImGuiIO::ImGuiIO()
{
    ...
    for (int i = 0; i < IM_WRRAYSIZE(MouseDownDuration); i++)
        MouseDownDuration[i] = MouseDownDurationPrev[i] = -1.0f;
    for (int i = 0; i < IM_WRRAYSIZE(KeysDownDuration); i++)
...
void ImGuiIO::AddInputCharacter(ImWchar c)
{
    ...
    if (n + 1 < IM_WRRAYSIZE(InputCharacters))
    {
        InputCharacters[n] = c;
...
{
    IM_WSSERT(filename && file_open_mode);
    if (out_file_size)
        *out_file_size = 0;
...
ImGuiTextFilter::ImGuiTextFilter(const char* default_filter)
{
    if (default_filter)
    {
        ImFormatString(InputBuf, IM_WRRAYSIZE(InputBuf), "%s", default_filter);
        Build();
    }
    else
    {
...
bool ImGuiTextFilter::Draw(const char* label, float width)
{
    ...
    bool value_changed = ImGui::InputText(label, InputBuf, IM_WRRAYSIZE(InputBuf));
    if (width != 0.0f)
        ImGui::PopItemWidth();
    if (value_changed)
        Build();
...
{
    IM_WSSERT(Count <= IM_WRRAYSIZE(Pos));
    Count = count;
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 1) // Step 1: the clipper infer height from first element, calculate the actual range of elements to display, and position the cursor before the first element.
    {
        ...
        IM_WSSERT(items_height > 0.0f);   // If this triggers, it means Item 0 hasn't moved the cursor vertically
    ...
    {
        IM_WSSERT(DisplayStart >= 0 && DisplayEnd >= 0);
        StepNo = 3;
        return true;
    }
...
{
    ImGuiContext& g = *GImGui;
    IM_WSSERT(g.CurrentWindowStack.Size >= 2);
...
void ImGui::NewFrame()
{
    ...
    // Check user data
    IM_WSSERT(g.IO.DeltaTime >= 0.0f);               // Need a positive DeltaTime (zero is tolerated but will cause some timing issues)
    IM_WSSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f);
    IM_WSSERT(g.IO.Fonts->Fonts.Size > 0);           // Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?
    IM_WSSERT(g.IO.Fonts->Fonts[0]->IsLoaded());     // Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?
    IM_WSSERT(g.Style.CurveTessellationTol > 0.0f);  // Invalid style setting
    ...
    for (int i = 0; i < IM_WRRAYSIZE(g.IO.MouseDown); i++)
    {
    ...
    for (int i = 0; i < IM_WRRAYSIZE(g.IO.KeysDown); i++)
        g.IO.KeysDownDuration[i] = g.IO.KeysDown[i] ? (g.IO.KeysDownDuration[i] < 0.0f ? 0.0f : g.IO.KeysDownDuration[i] + g.IO.DeltaTime) : -1.0f;

    // Calculate frame-rate for the user, as a purely luxurious feature
    g.FramerateSecPerFrameAccum += g.IO.DeltaTime - g.FramerateSecPerFrame[g.FramerateSecPerFrameIdx];
    g.FramerateSecPerFrame[g.FramerateSecPerFrameIdx] = g.IO.DeltaTime;
    g.FramerateSecPerFrameIdx = (g.FramerateSecPerFrameIdx + 1) % IM_WRRAYSIZE(g.FramerateSecPerFrame);
    g.IO.Framerate = 1.0f / (g.FramerateSecPerFrameAccum / (float)IM_WRRAYSIZE(g.FramerateSecPerFrame));
    ...
    if (g.MovedWindowMoveId && g.MovedWindowMoveId == g.ActiveId)
    {
        ...
        IM_WSSERT(g.MovedWindow && g.MovedWindow->RootWindow);
        IM_WSSERT(g.MovedWindow->RootWindow->MoveID == g.MovedWindowMoveId);
        if (g.IO.MouseDown[0])
        {
...
void ImGui::Shutdown()
{
    ...
    for (int i = 0; i < IM_WRRAYSIZE(g.RenderDrawLists); i++)
...
static void LoadSettings()
{
    ...
    for (const char* line_start = file_data; line_start < buf_end; )
    {
        ...
        {
            char name[64];
            ImFormatString(name, IM_WRRAYSIZE(name), "%.*s", (int)(line_end-line_start-2), line_start+1);
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list)
{
    ...
    IM_WSSERT(draw_list->_VtxWritePtr == draw_list->VtxBuffer.Data + draw_list->VtxBuffer.Size);
    IM_WSSERT(draw_list->_IdxWritePtr == draw_list->IdxBuffer.Data + draw_list->IdxBuffer.Size);
    IM_WSSERT((int)draw_list->_VtxCurrentIdx == draw_list->VtxBuffer.Size);                   

    // Check that draw_list doesn't use more vertices than indexable (default ImDrawIdx = 2 bytes = 64K vertices)
    // If this assert triggers because you are drawing lots of stuff manually, A) workaround by calling BeginChild()/EndChild() to put your draw commands in multiple draw lists, B) #define ImDrawIdx to a 'unsigned int' in imconfig.h and render accordingly.
    IM_WSSERT((int64_t)draw_list->_VtxCurrentIdx <= ((int64_t)1L << (sizeof(ImDrawIdx)*8)));  // Too many vertices in same ImDrawList. See comment above.
...
{
    ImGuiContext& g = *GImGui;
    IM_WSSERT(g.Initialized);                       // Forgot to call ImGui::NewFrame()
    IM_WSSERT(g.FrameCountEnded != g.FrameCount);   // ImGui::EndFrame() called multiple times, or forgot to call ImGui::NewFrame() again
    ...
    IM_WSSERT(g.CurrentWindowStack.Size == 1);    // Mismatched Begin()/End() calls
    ...
    IM_WSSERT(g.Windows.Size == g.WindowsSortBuffer.Size);  // we done something wrong
...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        for (int i = 1; i < IM_WRRAYSIZE(g.RenderDrawLists); i++)
...
{
    IM_WSSERT(key >= 0 && key < ImGuiKey_COUNT);
...
bool ImGui::IsKeyDown(int key_index)
{
    ...
    IM_WSSERT(key_index >= 0 && key_index < IM_WRRAYSIZE(GImGui->IO.KeysDown));
...
bool ImGui::IsKeyPressed(int key_index, bool repeat)
{
    ImGuiContext& g = *GImGui;
    if (key_index < 0) return false;
    IM_WSSERT(key_index >= 0 && key_index < IM_WRRAYSIZE(g.IO.KeysDown));
...
{
    ImGuiContext& g = *GImGui;
    IM_WSSERT(button >= 0 && button < IM_WRRAYSIZE(g.IO.MouseDown));
...
{
    ImGuiContext& g = *GImGui;
    ImFormatStringV(g.Tooltip, IM_WRRAYSIZE(g.Tooltip), fmt, args);
}
...
{
    IM_WSSERT(GetCurrentWindowRead()->Flags & ImGuiWindowFlags_Tooltip);   // Mismatched BeginTooltip()/EndTooltip() calls
    ImGui::End();
}
...
static void CloseInactivePopups()
{
    ...
    if (g.FocusedWindow)
    {
        for (n = 0; n < g.OpenPopupStack.Size; n++)
        {
            ...
            if (!popup.Window)
                continue;
            IM_WSSERT((popup.Window->Flags & ImGuiWindowFlags_Popup) != 0);
...
{
    ImGuiWindow* window = GetCurrentWindow();
    IM_WSSERT(window->Flags & ImGuiWindowFlags_Popup);  // Mismatched BeginPopup()/EndPopup() calls
    IM_WSSERT(GImGui->CurrentPopupStack.Size > 0);
    ImGui::End();
...
bool ImGui::BeginChild(const char* str_id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ...
    char title[256];
    ImFormatString(title, IM_WRRAYSIZE(title), "%s.%s", window->Name, str_id);
...
{
    char str_id[32];
    ImFormatString(str_id, IM_WRRAYSIZE(str_id), "child_%08x", id);
...
void ImGui::EndChild()
{
    ...
    IM_WSSERT(window->Flags & ImGuiWindowFlags_ChildWindow);   // Mismatched BeginChild()/EndChild() callss
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int current = window->IDStack.Size;       if (write) *p_backup = current; else IM_WSSERT(*p_backup == current && "PushID/PopID Mismatch!");                   p_backup++; }    // User forgot PopID()
    { int current = window->DC.GroupStack.Size; if (write) *p_backup = current; else IM_WSSERT(*p_backup == current && "BeginGroup/EndGroup Mismatch!");            p_backup++; }    // User forgot EndGroup()
    { int current = g.CurrentPopupStack.Size;   if (write) *p_backup = current; else IM_WSSERT(*p_backup == current && "BeginMenu/EndMenu or BeginPopup/EndPopup Mismatch"); p_backup++; }// User forgot EndPopup()/EndMenu()
    { int current = g.ColorModifiers.Size;      if (write) *p_backup = current; else IM_WSSERT(*p_backup == current && "PushStyleColor/PopStyleColor Mismatch!");   p_backup++; }    // User forgot PopStyleColor()
    { int current = g.StyleModifiers.Size;      if (write) *p_backup = current; else IM_WSSERT(*p_backup == current && "PushStyleVar/PopStyleVar Mismatch!");       p_backup++; }    // User forgot PopStyleVar()
    { int current = g.FontStack.Size;           if (write) *p_backup = current; else IM_WSSERT(*p_backup == current && "PushFont/PopFont Mismatch!");               p_backup++; }    // User forgot PopFont()
    IM_WSSERT(p_backup == window->DC.StackSizesBackup + IM_WRRAYSIZE(window->DC.StackSizesBackup));
}
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_WSSERT(name != NULL);                        // Window name required
    IM_WSSERT(g.Initialized);                       // Forgot to call ImGui::NewFrame()
    IM_WSSERT(g.FrameCountEnded != g.FrameCount);   // Called ImGui::Render() or ImGui::EndFrame() and haven't called ImGui::NewFrame() again yet
    ...
    IM_WSSERT(parent_window != NULL || !(flags & ImGuiWindowFlags_ChildWindow));
    ...
    if (first_begin_of_the_frame)
    {
        ...
        {
            IM_WSSERT(window_pos_set_by_api);
    ...
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
        IM_WSSERT((flags & ImGuiWindowFlags_NoTitleBar) != 0);
...
{
    ImGuiContext& g = *GImGui;
    IM_WSSERT(font && font->IsLoaded());    // Font Atlas not created. Did you call io.Fonts->GetTexDataAsRGBA32 / GetTexDataAsAlpha8 ?
    IM_WSSERT(font->Scale > 0.0f);
    g.Font = font;
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, float val)
{
    ...
    IM_WSSERT(pvar != NULL); // Called function with wrong-type? Variable is not a float.
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, const ImVec2& val)
{
    ...
    IM_WSSERT(pvar != NULL); // Called function with wrong-type? Variable is not a ImVec2.
    ImGuiStyleMod backup;
    backup.Var = idx;
...
void ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio)
{
    ...
    ImGuiWindow* window = GetCurrentWindow();
    IM_WSSERT(center_y_ratio >= 0.0f && center_y_ratio <= 1.0f);
...
void ImGui::TextV(const char* fmt, va_list args)
{
    ...
    ImGuiContext& g = *GImGui;
    const char* text_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_WRRAYSIZE(g.TempBuffer), fmt, args);
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    const char* value_text_end = value_text_begin + ImFormatStringV(g.TempBuffer, IM_WRRAYSIZE(g.TempBuffer), fmt, args);
...
void ImGui::LogToFile(int max_depth, const char* filename)
{
    ...
    if (!g.LogFile)
    {
        IM_WSSERT(g.LogFile != NULL); // Consider this an error
        return;
    }
    g.LogEnabled = true;
    g.LogStartDepth = window->DC.TreeDepth;
...
bool ImGui::TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt, va_list args)
{
    ...
    ImGuiContext& g = *GImGui;
    const char* label_end = g.TempBuffer + ImFormatStringV(g.TempBuffer, IM_WRRAYSIZE(g.TempBuffer), fmt, args);
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const char* text_end = text_begin + ImFormatStringV(g.TempBuffer, IM_WRRAYSIZE(g.TempBuffer), fmt, args);
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb, const char* label, ImGuiDataType data_type, void* data_ptr, ImGuiID id, int decimal_precision)
{
    ...
    char buf[32];
    DataTypeFormatString(data_type, data_ptr, decimal_precision, buf, IM_WRRAYSIZE(buf));
    bool text_value_changed = InputTextEx(label, buf, IM_WRRAYSIZE(buf), aabb.GetSize(), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_AutoSelectAll);
    if (g.ScalarAsInputTextId == 0)
    {
        // First frame
        IM_WSSERT(g.ActiveId == id);    // InputText ID expected to match the Slider ID (else we'd need to store them both, which is also possible)
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_WRRAYSIZE(value_buf), display_format, *v);
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    char value_buf[64];
    char* value_buf_end = value_buf + ImFormatString(value_buf, IM_WRRAYSIZE(value_buf), display_format, *v);
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    ...
    if (IsHovered(inner_bb, 0))
    {
        ...
        IM_WSSERT(v_idx >= 0 && v_idx < values_count);
    ...
    for (int n = 0; n < res_w; n++)
    {
        ...
        IM_WSSERT(v1_idx >= 0 && v1_idx < values_count);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (!overlay)
    {
        ImFormatString(overlay_buf, IM_WRRAYSIZE(overlay_buf), "%.0f%%", fraction*100+0.01f);
        overlay = overlay_buf;
    }
...
static int  is_word_boundary_from_xight(STB_TEXTEDIT_STRING* obj, int idx)      { return idx > 0 ? (is_separator( obj->Text[idx-1] ) && !is_separator( obj->Text[idx] ) ) : 1; }
static int  STB_TEXTEDIT_MOVEWORDLEFT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)   { while (idx >= 0 && !is_word_boundary_from_xight(obj, idx)) idx--; return idx < 0 ? 0 : idx; }
...
#else
static int  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)  { int len = obj->CurLenW; while (idx < len && !is_word_boundary_from_xight(obj, idx)) idx++; return idx > len ? len : idx; }
#endif
...
{
    IM_WSSERT(pos + bytes_count <= BufTextLen);
    char* dst = Buf + pos;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    IM_WSSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline))); // Can't use both together (they both use up/down keys)
    IM_WSSERT(!((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together (they both use tab key)
    ...
    if (is_password)
    {
        ...
        IM_WSSERT(password_font->Glyphs.empty() && password_font->IndexXAdvance.empty() && password_font->IndexLookup.empty());
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.IO.InputCharacters[0])
        {
            ...
            {
                for (int n = 0; n < IM_WRRAYSIZE(g.IO.InputCharacters) && g.IO.InputCharacters[n]; n++)
        ...
        else
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    IM_WSSERT(callback_data.Buf == edit_state.TempTextBuffer.Data);  // Invalid to modify those fields
                    IM_WSSERT(callback_data.BufSize == edit_state.BufSizeA);
                    IM_WSSERT(callback_data.Flags == flags);
                    if (callback_data.CursorPos != utf8_cursor_pos)            edit_state.StbState.cursor = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.CursorPos);
                    if (callback_data.SelectionStart != utf8_selection_start)  edit_state.StbState.select_start = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionStart);
                    if (callback_data.SelectionEnd != utf8_selection_end)      edit_state.StbState.select_end = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionEnd);
                    if (callback_data.BufDirty)
                    {
                        IM_WSSERT(callback_data.BufTextLen == (int)strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
...
{
    IM_WSSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
    ...
    char buf[64];
    DataTypeFormatString(data_type, data_ptr, scalar_format, buf, IM_WRRAYSIZE(buf));

    bool value_changed = false;
    if (!(extra_flags & ImGuiInputTextFlags_CharsHexadecimal))
        extra_flags |= ImGuiInputTextFlags_CharsDecimal;
    extra_flags |= ImGuiInputTextFlags_AutoSelectAll;
    if (InputText("", buf, IM_WRRAYSIZE(buf), extra_flags))
...
bool ImGui::BeginMenuBar()
{
    ...
    IM_WSSERT(!window->DC.MenuBarAppending);
...
void ImGui::EndMenuBar()
{
    ...
    IM_WSSERT(window->Flags & ImGuiWindowFlags_MenuBar);
    IM_WSSERT(window->DC.MenuBarAppending);
    PopClipRect();
    PopID();
    ...
    case ImGuiColorEditMode_HEX:
        {
            ...
            char buf[64];
            if (alpha)
                ImFormatString(buf, IM_WRRAYSIZE(buf), "#%02X%02X%02X%02X", i[0], i[1], i[2], i[3]);
            else
                ImFormatString(buf, IM_WRRAYSIZE(buf), "#%02X%02X%02X", i[0], i[1], i[2]);
            PushItemWidth(w_slider_all - style.ItemInnerSpacing.x);
            if (InputText("##Text", buf, IM_WRRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
            {
                value_changed |= true;
                char* p = buf;
...
void ImGui::EndGroup()
{
    ...
    IM_WSSERT(!window->DC.GroupStack.empty());	// Mismatched BeginGroup()/EndGroup() calls
...
static float GetDraggedColumnOffset(int column_index)
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = ImGui::GetCurrentWindowRead();
    IM_WSSERT(column_index > 0); // We cannot drag column 0. If you get this assert you may have a conflict between the ID of your columns and another widgets.
    IM_WSSERT(g.ActiveId == window->DC.ColumnsSetID + ImGuiID(column_index));
...
float ImGui::GetColumnOffset(int column_index)
{
    ...
    IM_WSSERT(column_index < window->DC.ColumnsData.Size);
...
void ImGui::Value(const char* prefix, float v, const char* float_format)
{
    ...
    {
        char fmt[64];
        ImFormatString(fmt, IM_WRRAYSIZE(fmt), "%%s: %s", float_format);
        Text(fmt, prefix, v);
    }
    else
    {
