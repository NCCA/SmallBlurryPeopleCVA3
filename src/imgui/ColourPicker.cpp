...
{
    return   ImVec4(_c.m_x,_c.m_y,_c.m_z,_c.m_w);
}
...
bool ColorSelector(const char* pLabel, ngl::Vec4 &oRGBA)
{
    ...
    if (ImGui::InvisibleButton("Picker", ImVec2(16,16)))
    {
        ...
        pWindow->StateStorage.SetFloat(iStorageStartColorR, oRGBA.m_x);
        pWindow->StateStorage.SetFloat(iStorageStartColorG, oRGBA.m_y);
        pWindow->StateStorage.SetFloat(iStorageStartColorB, oRGBA.m_z);
        pWindow->StateStorage.SetFloat(iStorageStartColorA, oRGBA.m_w);

        float fHue, fSat, fVal;
        ImGui::ColorConvertRGBtoHSV( oRGBA.m_x, oRGBA.m_y, oRGBA.m_z, fHue, fSat, fVal );
        ...
        pWindow->StateStorage.SetFloat(iStorageCurrentColorA, oRGBA.m_w);
    }
    ...
    if (pWindow->StateStorage.GetInt(iStorageOpen, 0) == 1 && ImGui::Begin("Color picker", NULL, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::Separator();
        {
            //Saturation
            {
                ...
                if (ImGui::IsItemActive())
                {
                    ...
                    ImGui::ColorConvertHSVtoRGB( fHue, fSat, fVal, oRGBA.m_x, oRGBA.m_y, oRGBA.m_z );
        ...
        ImGui::Separator();
        {
            int r = (int)(ImSaturate( oRGBA.m_x )*255.f);
            int g = (int)(ImSaturate( oRGBA.m_y )*255.f);
            int b = (int)(ImSaturate( oRGBA.m_z )*255.f);
            int a = (int)(ImSaturate( oRGBA.m_w )*255.f);
            bool bChange = false;
            ...
            {
                bRet = true;
                oRGBA.m_x = (float)r/255.f;
                oRGBA.m_y = (float)g/255.f;
                oRGBA.m_z = (float)b/255.f;
                oRGBA.m_w = (float)a/255.f;
        ...
        {
            pWindow->StateStorage.SetInt(iStorageOpen, 0);
            oRGBA.m_x = pWindow->StateStorage.GetFloat(iStorageStartColorR);
            oRGBA.m_y = pWindow->StateStorage.GetFloat(iStorageStartColorG);
            oRGBA.m_z = pWindow->StateStorage.GetFloat(iStorageStartColorB);
            oRGBA.m_w = pWindow->StateStorage.GetFloat(iStorageStartColorA);
    ...
    float fValues[4] = {oRGBA.m_x, oRGBA.m_y, oRGBA.m_z, oRGBA.m_w};
