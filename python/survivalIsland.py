...
for x in range(map_width):
  for y in range(map_height):
    noise = int(gen.fractal(x, y, octaves, persistence, freq, min, max)) + 80
    noise *= pow((max_len - hf.len(x - half_width, y - half_height))/max_len, 2)
    ...
    if noise > mountain_height:
      ...
      map_data[x + map_width * y][1] = int(gen.fractal(x, y, octaves, persistence, freq/2, min, max)) + 100
